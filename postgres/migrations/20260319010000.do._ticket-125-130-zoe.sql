-- TICKET-125: reporting_readonly role
-- TICKET-130: zoe_api_keys table
-- TICKET-131: zoe_config table (prompt + model management)

-- ============================================================
-- Read-only role for Zoe DB access
-- ============================================================
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'reporting_readonly') THEN
        CREATE ROLE reporting_readonly WITH LOGIN PASSWORD 'change_me_in_production';
    END IF;
END
$$;

GRANT CONNECT ON DATABASE automator TO reporting_readonly;
...
GRANT SELECT ON
    ...
    trash_reasons,
    call_outcomes,
    call_request_reasons
TO reporting_readonly;

-- Explicitly revoke sensitive tables
REVOKE ALL ON users FROM reporting_readonly;
REVOKE ALL ON user_permissions FROM reporting_readonly;
