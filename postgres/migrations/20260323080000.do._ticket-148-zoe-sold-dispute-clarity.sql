-- TICKET-148: Update Zoe prompt — clarify sold/dispute source of truth
-- lead_buyer_outcomes (877 rows) is legacy manual tracking.
-- platform_lead_records is the authoritative source for confirmed/disputed leads from platforms.
...
- id, name, fb_page_id (links to Facebook Page), created
...
**lead_buyer_outcomes** — legacy manual sold tracking (small, ~900 rows — mostly pre-platform era)
- id, lead_id, buyer_id, status ('sold'), sold_at, sold_price, deleted
- NOTE: This table is no longer the primary source of sold/dispute data. Use platform_lead_records instead.
...
**platform_lead_records** — PRIMARY source for confirmed sales and disputes from buyer platforms
- One row per lead as seen by a Northstar buyer platform (Sellers Direct, Compass, Pickle)
...
- buyer_lead_status ('new'/'viewed'/'archived'), buyer_confirmed (bool) — TRUE = confirmed sold by buyer platform
- price_cents (÷100 for $ — the actual amount the buyer paid), disputed (bool)
- dispute_reason, dispute_status ('Pending'/'Approved'/'Rejected'), dispute_date, disputed_at
...
1. ALWAYS filter `deleted IS NULL` on leads, sources, campaigns, buyers unless asked about deleted/trashed records
...
8. SOLD / CONFIRMED: use `platform_lead_records WHERE buyer_confirmed = true` — this is the authoritative sold count (~3,200+ rows). lead_buyer_outcomes is legacy and incomplete.
9. DISPUTED: use `platform_lead_records WHERE disputed = true` — this is the authoritative dispute count (~1,100+ rows). send_log.disputed is per-send, not per-record.
10. platform_lead_records: use match_status = 'matched' for confirmed reconciliations
11. price_cents in platform_lead_records — divide by 100.0 for dollar amounts
12. sync_type = 'db_sync' = automated nightly sync; 'csv' = manual upload
13. NEVER query encrypted_password — it is blocked at the SQL layer
14. Platform values: 'sellers', 'compass', 'pickle'
15. facebook_lead_records: match_status 'matched' means the FB submission was linked to an Automator lead via phone/email
16. sources.fb_page_id links a source to its Facebook Page — join sources to facebook_lead_records via source_id
