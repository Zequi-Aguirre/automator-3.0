...
export type UserInviteEmailData = {
    name: string;
    email: string;
    tempPassword: string;
    appUrl?: string;
...
export function userInviteEmail(data: UserInviteEmailData): { subject: string; html: string } {
    const { name, email, tempPassword, appUrl } = data;
    const loginUrl = appUrl ?? 'https://app.automator.io';

    const content = `
      <h2 style="margin:0 0 8px;color:#111827;font-size:22px;font-weight:700;">Welcome to Automator</h2>
      <p style="margin:0 0 24px;color:#6b7280;font-size:14px;">Hi ${name}, your account has been created. Use the credentials below to log in.</p>
            ...
            <table width="100%" cellpadding="0" cellspacing="0">
              ...
              <tr>
                <td style="padding-top:14px;padding-bottom:4px;">
                  <span style="color:#9ca3af;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;">Temporary Password</span>
                </td>
              </tr>
              <tr>
                <td>
        ...
        <tr>
          <td style="padding:14px 18px;">
            <p style="margin:0;color:#92400e;font-size:13px;line-height:1.5;">
              <strong>You will be required to set a new password</strong> the first time you log in. Your temporary password cannot be used after that.
            </p>
          </td>
        </tr>
      </table>
    ...
    return {
        subject: 'Your Automator account is ready',
